...
import { api } from "@/lib/api";
import { Plus, Users, Calendar, FileText, FolderOpen, X, Upload, User } from "lucide-react";
import Link from "next/link";
import { useToast } from "@/hooks/use-toast";
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        ...
        console.error("Failed to fetch projects:", error);
        toast({ title: "Demo Mode", description: "Using mock data - connect to your backend API" });
      } finally {
        setLoading(false);
      }
    };
          ...
          <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-xl shadow-xl">
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>Fill in the project details and assign team members</CardDescription>
              </div>
            ...
            <CardContent>
              <form onSubmit={(e) => {
                ...
                const selected = users.filter((u) => formData.getAll("users").includes(u.id));
                handleCreateProject({
                ...
                <div className="space-y-2">
                  ...
                  <textarea name="description" required className="w-full p-2 border rounded min-h-[80px]" />
                </div>
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    {users.map((user) => (
                      <div key={user.id} className="flex items-center space-x-3 p-3 border rounded-lg hover:bg-gray-50">
                        <Checkbox name="users" value={user.id} id={`user-${user.id}`} />
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {Array.isArray(projects) && projects.map((project) => (
            <Card key={project.id} className="hover:shadow-lg border border-muted transition-shadow rounded-xl">
              <CardHeader>
